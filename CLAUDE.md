# Andy Issues — Development Guide

## Project Overview

Andy Issues is an Andy ecosystem microservice: Issues management service
...
|-------|---------|---------|
| Domain | `Andy.Issues.Domain` | Entities, enums, value objects — no dependencies |
| Application | `Andy.Issues.Application` | Interfaces, DTOs, contracts |
| Infrastructure | `Andy.Issues.Infrastructure` | EF Core, service implementations, external integrations |
| API | `Andy.Issues.Api` | REST controllers, MCP tools, gRPC services, auth, middleware |
| Shared | `Andy.Issues.Shared` | Types shared across projects |
| CLI | `Andy.Issues.Cli` | Command-line interface (System.CommandLine) |
...
ASPNETCORE_ENVIRONMENT=Development dotnet run --project src/Andy.Issues.Api

# Run the API with SQLite
Database__Provider=Sqlite dotnet run --project src/Andy.Issues.Api
...
# CLI
dotnet run --project tools/Andy.Issues.Cli -- items list

# EF migrations (create)
dotnet ef migrations add MigrationName --project src/Andy.Issues.Infrastructure --startup-project src/Andy.Issues.Api

# EF migrations (apply)
dotnet ef database update --project src/Andy.Issues.Infrastructure --startup-project src/Andy.Issues.Api
```
...
- **RBAC**: Application code `andy-issues` registered in Andy RBAC with admin/user/viewer roles
